...
この節の旧方針「LabNAT の NetNat を撤去」は、L2 がインターネットへ出られないため廃止した。
現在は **L1 + L0 の二段 NAT** と、制御 VM 用の静的 KDC/管理ポートマッピングを併用する。
詳細と実証結果は [KB/0021](0021-l2-double-nat-management.md) を参照。

旧方針で有効だった要素（L1 の IP forwarding、制御 VM の静的ルート、初期構成時の
PowerShell Direct）は引き続き利用する。

<!-- Historical details retained below for the failure analysis. -->
- ~~LabNAT から NetNat を撤去し、純粋なルーティングにする。~~
...
現在は静的マッピング経由で「制御VM が Ansible で L2 を直接たたく」構成を維持する。
